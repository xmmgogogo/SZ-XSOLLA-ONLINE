<?php
/**
 * <!--新版随机生成订单功能，主要需要单独配置一份随机池子，然后从池子里选择出结果-->
 */

//下载当前页面数据
if(isset($_POST['downLoadCsv']) && isset($_POST['csvData'])) {
    export_csv(date('Ymd') . '-GAME-2.0(' . $_POST['gameName'] . ')' .  '.csv', trim($_POST['csvData']));die; //导出
} else {
    if(empty($_POST)) {
...
//    var_dump($todayEnd - $todayStart);die;
    ...
    foreach($paymentType as $eT) {
        ...
        //金额随机值
        $eTList['moneyRankList'] = array_merge($eTList['moneyRankList'], randPayTask($payAmount, $eT));
    }
    ...
    //时间随机值
    $timeRankList = randPayTask_time($todayTotalSecond, count($eTList['moneyRankList']), isset($_POST['rand-time']) ? floatval($_POST['rand-time'] / 10000) : 0);
 ...
 * @return array
 */
function randPayTask($payAmount, $eT = 'big') {
    if(!$payAmount) {
        return [];
    }
    ...
    //计算开始
    if(file_exists('payAmountPondConf/' . $eT . '.php')) {
        $config = require('payAmountPondConf/' . $eT . '.php');
    } else {
        die('payAmountPondConf/' . $eT . '.php not exists!!!!!');
    }

    $minNum = min($config);
    while($payAmount && $payAmount > $minNum) {
        $randNum = roll($config);
        $pieceMoneyList[] = $randNum;
 ...
 * @return array
 */
function randPayTask_time($payAmount, $timeSplit, $rand) {
...
<div style="width: 1000px; margin: 10px auto;">
    <h3><font color="red">报单定时任务系统 1.1  -  生成的全部订单</font> - 单独配置池(payAmountPondConf/*.txt)</h3><BR>

    <div style="border: 1px solid #ccc;">
        <form class="form-horizontal" name= "randPayTask-form" action="payTask_fixed_pond.php" method="post" target="_blank">

            <div class="form-group" style="padding-top: 10px;">
                <label class="col-sm-2 control-label"><span style="color: red;font-weight: bold;">当前总金额：~<?php echo isset($eTList['moneyRankList']) ? array_sum($eTList['moneyRankList']) : 0;?></span></label>
